...
export function SiteFooter() {
  return (
    ...
    >
      ...
      <div
        className="pointer-events-none absolute inset-x-0 bottom-0 top-1/3 bg-[radial-gradient(ellipse_85%_55%_at_50%_100%,rgba(234,88,12,0.09),transparent_58%)] dark:bg-[radial-gradient(ellipse_85%_55%_at_50%_100%,rgba(251,146,60,0.12),transparent_58%)]"
        aria-hidden
      />
      <div className="relative z-10 mx-auto flex max-w-content flex-col gap-8 px-4 text-center sm:px-6 lg:px-8">
        <div className="flex w-full flex-col items-center gap-8 sm:flex-row sm:flex-wrap sm:justify-center sm:gap-x-10 sm:gap-y-4 sm:text-left">
          <p className="whitespace-nowrap text-sm text-muted-dark">
            © {new Date().getFullYear()} Deepanshu Chauhan.{" "}
            <span className="inline font-mono text-[10px] text-muted-dark/95">
              v{PORTFOLIO_VERSION}
            </span>
          </p>
          <FooterSocialBar />
