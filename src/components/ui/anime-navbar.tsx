...
export function AnimeNavBar({ items, className, defaultActive = "Home" }: NavBarProps) {
  ...
  const scrollToSection = (sectionId: string) => {
    // Special handling for bug bounty section
    const targetId = sectionId === 'bug-bounty' ? 'bug-bounty' : sectionId;
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = items.map(item => {
        const targetId = item.url === 'bug-bounty' ? 'bug-bounty' : item.url;
  ...
  return (
    <div className="fixed top-5 left-0 right-0 z-[9999]">
      <div className="flex justify-center pt-6">
        <motion.div 
          className="flex items-center gap-3 bg-black/50 border border-white/10 backdrop-blur-lg py-2 px-2 rounded-full shadow-lg relative glow-border"
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "relative text-sm font-semibold px-6 py-3 rounded-full transition-all duration-300",
